define([],function(){
    ...
    var objectMetadata;
    var context = {"object" : "productSearched", "objectService" : "bestBuyProducts"};
	...
	function productSearched(defaultValues){
		var privateState = {};
			context["field"]  = "sku";
			context["metadata"] = (objectMetadata ? objectMetadata["sku"] : null);
	...
	BaseModel.isParentOf(productSearched);
	
	//Create new class level validator object
	BaseModel.Validator.call(productSearched);
	
	var registerValidatorBackup = productSearched.registerValidator;
	
	productSearched.registerValidator = function(){
		var propName = arguments[0];
		if(!setterFunctions[propName].changed){
			var setterBackup = setterFunctions[propName];
			setterFunctions[arguments[0]] = function(){
				if( productSearched.isValid(this, propName, val) ){
	...
	productSearched.getProductsForSearchText = function(params, onCompletion){
		return productSearched.customVerb('getProductsForSearchText', params, onCompletion);
	};
	
	var relations = [
	];
	
	productSearched.relations = relations;
	
	productSearched.prototype.isValid = function(){
		return productSearched.isValid(this);
	};
	
	productSearched.prototype.objModelName = "productSearched";
	 ...
	 */
	productSearched.registerProcessors = function(options, successCallback, failureCallback) {
		...
		kony.mvc.util.ProcessorUtils.getMetadataForObject("bestBuyProducts", "productSearched", options, metaDataSuccess, metaDataFailure);
	};
	
	//clone the object provided in argument.
	productSearched.clone = function(objectToClone) {
		var clonedObj = new productSearched();
